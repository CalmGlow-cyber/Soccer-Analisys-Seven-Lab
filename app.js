...
function preparaDataset(grezzo){
  ...
  const presenze = (grezzo.Presenze_Allenamento || []).filter(r => !rigaVuota(r) && !isEsempio(r)).map(r => {
    ...
    const minuti = N(r.Minuti_Allenamento), rpe = N(r.RPE);
    return {
      ...
      Carico: pres ? minuti*rpe : 0, Data: s ? s.Data : null, Mese: s ? s.Mese : null,
  ...
  const nomiGiocatori = Array.from(new Set(giocatori.map(g=>g.Giocatore))).sort((a,b)=>a.localeCompare(b,"it"));
...
function aggregaAllenamenti(sessioni, presenze){
  ...
  pres.forEach(p => {
    ...
    g.n++; if(p.Presente){ g.presenti++; g.minuti += p.Minuti_Allenamento; g.rpe.push(p.RPE); }
    g.carico += p.Carico;
  });
...
function validaDati(ds){
  ...
  ds.presenze.forEach(p => {
    ...
    if(p.Presente && (p.RPE < 1 || p.RPE > 10))
...
function renderAllenamenti(f){
  ...
  if(!stato.ds.haAllenamenti){
    cont.innerHTML = `<div class="vuoto"><strong>Nel file non ci sono i fogli Allenamenti e Presenze_Allenamento.</strong><br>
      Aggiungili al tuo Excel (sessione, data, tipo, durata; e per ogni giocatore presenza, minuti e RPE percepito da 1 a 10) per vedere presenze e carico di lavoro. Il resto della dashboard funziona comunque.</div>`;
    return;
  }
  ...
  const mensile = caricoMensile(stato.ds.allenamenti, stato.ds.presenze);
  const sottoSoglia = righe.filter(r => r.Tasso_Presenza_pct !== null && r.Tasso_Presenza_pct < 60);
  const cgm = caricoGiocatoreMese(stato.ds.presenze);
  ...
  if(avvisi.length === 0) avvisi.push({tipo:"ok", testo:"Presenze sopra il 60% per tutti e nessun salto di carico pari o superiore a 1,5 volte tra un mese e l'altro."});
  ...
  cont.innerHTML = `
    <div class="griglia g-kpi" style="margin-bottom:16px">
      ...
      <div class="kpi"><div class="kpi-eti">Carico totale squadra</div><div class="kpi-valore">${nf0(righe.reduce((a,r)=>a+r.Carico_Tot,0))}</div><div class="kpi-nota">Unità sRPE (minuti × RPE)</div></div>
    ...
    <div class="griglia g-2" style="margin-bottom:16px">
      ...
      <div class="card"><div class="grafico-titolo">Carico di allenamento per giocatore</div>
      ...
      <div class="card"><div class="grafico-titolo">Carico medio di squadra mese per mese</div>
        ...
        <div class="grafico-wrap"><canvas id="gr-carico-mese"></canvas></div></div>
    </div>
    <div class="card">
      <div class="grafico-titolo">Segnali da valutare con lo staff</div>
      <div class="grafico-sub">Presenze sotto il 60% e aumenti di carico pari o superiori a 1,5 volte da un mese all'altro.</div>
...
function renderIncroci(){
  ...
  if(!stato.ds.haAllenamenti){
    cont.innerHTML = `<div class="vuoto"><strong>Servono sia le partite sia gli allenamenti</strong> per calcolare questo incrocio: carica anche i fogli Allenamenti e Presenze_Allenamento.</div>`;
    return;
  }
...
async function generaReportMensile(mese){
  ...
  const mensile = ds.haAllenamenti ? caricoMensile(ds.allenamenti, ds.presenze) : [];
  ...
  const incrociMese = incrociCaricoRendimento(ds).filter(x => x.meseCorr === mese);
  const piuCaricoGiocatore = righeAllen.slice().sort((a,b)=>b.Carico_Tot-a.Carico_Tot)[0];
  ...
  const variazioneGiocatore = (piuCaricoGiocatore && caricoPrecGiocatore && caricoPrecGiocatore.Carico_Tot) ? piuCaricoGiocatore.Carico_Tot/caricoPrecGiocatore.Carico_Tot : null;
  ...
  await generaEScarica(nomeFileData("Report_Mensile_"+mese.replace("-",""), new Date()), [
    async (pag) => {
      ...
      if(caricoMeseCorr) bullet.push(`Il carico medio di allenamento (sRPE) è stato di ${nf0(caricoMeseCorr.Carico_Medio)} unità a presenza${caricoMesePrec?`, contro ${nf0(caricoMesePrec.Carico_Medio)} del mese precedente`:""}.`);
      if(piuCaricoGiocatore) bullet.push(`<b>${esc(piuCaricoGiocatore.Giocatore)}</b> ha accumulato il carico più alto del mese (${nf0(piuCaricoGiocatore.Carico_Tot)} u.a.).`);
      bulletsReport(pag, bullet);
      ...
      if(rapportoCarico !== null && rapportoCarico >= 1.5) avvisi.push(`Il carico medio di squadra è aumentato del ${nf0((rapportoCarico-1)*100)}% rispetto al mese precedente: un salto così rapido è un segnale da monitorare con lo staff.`);
    ...
    Object.assign(async (pag) => {
      ...
      if(righeAllen.length){
        ...
        tabellaReport(pag, ["Giocatore","Ruolo","Sessioni (pres. / tot.)","Presenza","Minuti tot.","Carico tot. (sRPE)","RPE medio"],
          righeAllen.map(r => [esc(r.Giocatore), esc(ruoloDi(r.Giocatore)), `${nf0(r.Sessioni_Presenti)}/${nf0(r.Sessioni_Totali)}`, pctTxt(r.Tasso_Presenza_pct,0), nf0(r.Minuti_Tot), nf0(r.Carico_Tot), nf(r.RPE_Medio,1)]));
      } else {
...
async function generaReportStagionale(){
  ...
  const incroci = incrociCaricoRendimento(ds).slice(0,6);
  ...
  await generaEScarica(nomeFileData("Report_Stagionale", new Date()), [
    ...
    async (pag) => {
      ...
      bulletsReport(pag, testi.length ? testi : ["Nessun segnale incrociato ancora disponibile: servono almeno due mesi consecutivi con partite e allenamenti."]);
...
function contestoIA(){
  ...
  if(stato.ds.haAllenamenti){
    ...
    allen = {sessioni:a.totSess, giocatori:a.righe.map(r=>({giocatore:r.Giocatore,
      presenza_pct: r.Tasso_Presenza_pct===null?null:+r.Tasso_Presenza_pct.toFixed(0),
      carico_srpe:r.Carico_Tot, rpe_medio: r.RPE_Medio===null?null:+r.RPE_Medio.toFixed(1)})),
      carico_mensile: caricoMensile(stato.ds.allenamenti, stato.ds.presenze).map(m=>({mese:m.Mese, carico_totale:m.Carico_Tot, carico_medio:m.Carico_Medio===null?null:+m.Carico_Medio.toFixed(0)}))};
  }
  return {
...
async function chiediIA(){
  ...
  if(!stato.ds){ box.textContent = "Carica prima i dati (file Excel o dati di esempio): senza numeri non c'è nulla da analizzare."; return; }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("#input-file").addEventListener("change", e => { if(e.target.files[0]) gestisciFile(e.target.files[0]); });
  ...
  dz.addEventListener("drop", e => { const f = e.dataTransfer?.files?.[0]; if(f) gestisciFile(f); });
